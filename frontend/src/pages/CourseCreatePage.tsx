...
import { CourseCreate, CourseUpdate } from '../types/generated/courses';
import { apiClient } from '../services/apiClient';
import CourseTaskForm, { CourseTaskFormHandle } from '../components/CourseTaskForm';
import { FormPageLayout } from '../components/common/FormPageLayout';

const CourseCreatePage: React.FC = () => {
  const navigate = useNavigate();
  const { courseFamilyId } = useParams<{ courseFamilyId?: string }>();
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const formRef = useRef<CourseTaskFormHandle>(null);

  const handleFormSubmit = async (data: CourseCreate | CourseUpdate) => {
    try {
      setLoading(true);
      setError(null);
  ...
  return (
    <FormPageLayout
      title="Create Course"
      subtitle="Create a new course within a course family. GitLab integration will be automatically inherited from the organization."
      onBack={handleCancel}
